...
const StudyPackage = ({studyPackageData}) => {
  return (
    <div className='container mt-14 mb-[25px] lg:mb-[60px]' id="study-material">
      {studyPackageData?.length > 0 ? (
        <div>
          <h3 className='text-[28px] text-[rgb(34,22,56)] pl-5 font-medium'>Study Packages</h3>
          <div className='grid xl:grid-cols-3 md:grid-cols-2 gap-1'>
            {studyPackageData?.map((item, index) => {
              // Format start date
        ...
        <div className='border text-center py-5 shadow-lg'>
          <h5 className='text-[#221638] text-xl'>
            There are no Study Packages found..
          </h5>
        </div>
      )}
    </div>
