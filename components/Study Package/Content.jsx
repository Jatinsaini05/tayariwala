...
import { getProductData } from "../../service/apiFetch"; // Import the getProductData function
...
const Content = ({ courseData }) => {
    ...
    useEffect(() => {
        if (courseData?.length > 0) {
            setCurrentId(courseData[0]._id);
            setSelectedOption(courseData[0].valueAlias);
        }
    }, [courseData]);
    ...
    const fetchDefaultProductData = () => {
        const params = { course: currentId, category: '5b47f7ef1e899567332d1d0c', populate: 'stream' };
    ...
    const handleSearch = (e) => {
        ...
        if (searchQuery.trim()) {
            const params = {
                course: currentId,
                category: '5b47f7ef1e899567332d1d0c',
                populate: 'stream',
                title: searchQuery
            };
            getProductData(params)
