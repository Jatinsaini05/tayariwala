...
const LiveClassesdetail = ({ details }) => {
  ...
  listData?.forEach((item) => {
    if (item.list.shortCode.toLowerCase().includes("faqs")) {
      ...
      item.list.shortCode.toLowerCase().includes("feature") ||
    ...
    if (currId && subId) {
      const fetchLessons = async () => {
        const response = await fetch(
          `https://maheshpratap.institute.org.in/api/public/data/get-lessons-group-by-unit?curriculum=${currId}&subject=${subId}`
        );
          ...
          <div className="lg:w-2/3">
            <div className="flex text-[#212529] flex-wrap sm:flex-nowrap">
              {[
                ...
                <div className="relative w-1/2 sm:w-auto" key={idx}>
                  <button
                    className={`border xl:w-40 w-full sm:w-32 py-[18px] ${
                      section === sec
                  ...
                  >
                    {sec.charAt(0).toUpperCase() +
                      sec.slice(1).replace("_", " ")}
                  </button>
                  <div
                    className={`w-full bg-customFC6200 absolute top-0 ${section === sec ? "h-1" : ""}`}
                  ></div>
                </div>
              ))}
            </div>
            <div className="mt-[56px] mb-[40px]">
              {section === "About Program" && (
                <div>
                  <span
                    dangerouslySetInnerHTML={{ __html: details?.desci }}
                  ></span>
                  <div className="my-5">
                    <h3 className="text-customFC6200 mb-3 text-lg">
                      Program Features
                    </h3>
                  ...
                  <span
                    dangerouslySetInnerHTML={{ __html: details?.more?.content }}
                  ></span>
                </div>
              )}
                            ...
                            lessonData.map((item, index) => (
                              <div key={index} className="mb-3">
                                ...
                                {item?.lessons?.map((itm, lessonIndex) => (
                                  <div
                                    key={lessonIndex}
                                    className="bg-[#f1f2f866] mb-1 sm:px-7 px-5"
                                  >
              ...
              {section === "Fee Structure" && (
                <div className="desci">
                  <InnerContentBlock
                    innerData={details?.courseStructure?.content}
                  ></InnerContentBlock>
                </div>
              )}
              {section === "FAQs" && (
                <div className="">
                  {faqs?.map((faq, idx) => (
                    <div
                      key={idx}
                      className={`mb-2  border rounded overflow-hidden ${accordOpen === idx ? "border-customFC6200" : ""}`}
                    >
                      <div
                        onClick={() =>
                          setAccordOpen(accordOpen === idx ? null : idx)
                        }
                        className="px-3 cursor-pointer bg-[#f7f7f7]  border-b-2 py-4 gap-2 flex justify-between items-center"
                      >
                        <h2 className="text-[#212529]">{faq?.title}</h2>
                        <span>
                          {accordOpen == idx ? (
                      ...
                      {accordOpen === idx && (
                        <div className="py-4  text-[#606060] px-3">
                          <p>
                            {" "}
                            <span
                              dangerouslySetInnerHTML={{ __html: faq?.desci }}
                            ></span>
                          </p>
                        </div>
                      )}
              ...
              <div className="pb-[10px] sticky top-0 border-b">
                <div className="flex items-center gap-5">
                  <div className=" flex items-center ">
                    ...
                    <span className="text-[#343a40] text-[30px] pl-1 font-bold">
                      {apiData?.cost}
                    </span>
                  </div>
                  <div className="text-sm flex items-center font-bold gap-1">
                    <FaRupeeSign className="text-xs text-customFC6200" />
                    <span className="line-through text-[#9b9b9b]">
                      {apiData?.mrp}
                    </span>
                    <span>{`${Math.floor(100 - (apiData?.cost * 100) / apiData?.mrp)}% Off`}</span>
                  </div>
                </div>
                <div className="my-3 text-[13px] font-semibold text-center text-[#dc3545] border border-[#dc3545]">
                  <span>
                    Offer ends on {apiData?.customParam?.offerEndDate}
                  </span>
                </div>
              </div>
            ...
            <div className="relative">
              <div className="group">
                <div className="flex gap-2 hover:cursor-pointer justify-center items-center py-[10px] px-[25px] font-semibold mt-[10px]">
                  <span className="text-customFC6200">Share This Course</span>
                  <TbShare />
                </div>
