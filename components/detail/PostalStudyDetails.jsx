...
const PostalStudyDetails = ({ aboutProgram, examInfo, FAQ, details }) => {
    ...
    const handleButtonClick = () => {
        Swal.fire({
            text: "Book sample will be available soon...",
            icon: "info",
            timer: 3000,
                ...
                <div className='flex flex-col-reverse md:flex-col lg:flex-row justify-between gap-8'>
                    <div className='lg:w-2/3'>
                        <div className='flex text-[#212529] flex-wrap sm:flex-nowrap'>
                            {["About Program", "Books", "Exam Info", "FAQs"].map((sec, idx) => (
                            ...
                            {section === "Books" && (
                                <div className=''>
                                    <div className='grid md:grid-cols-3 grid-cols-1 gap-6'>
                                        {apiData?.comboProducts?.map((item, idx) => (
                                            <>
                                                <div key={idx} className='border shadow-[0px_16px_14px_9px_rgba(0,_0,_0,_0.1)] rounded w-fit'>
                                                    <div className='py-3 bg-[#f7f7f7] min-h-[100px] flex flex-col justify-between text-center px-5'>
                                                        <div className='text-[#606060] font-semibold'>
                                                            {item?.title}
                                                        </div>
                                                    ...
                                                    <div className='border-b overflow-hidden border-t group flex justify-center'>
                                                        <div className='px-5'><img src={item?.book?.thumb} alt="book image" className='md:w-48 group-hover:scale-[1.4]'></img>
                                                        </div>
                                                    </div>
                                                    <div className='text-center py-3'>
                                                        <div>
                                                            {item?.book?.sample ? (
                                                                <Link
                                                                    href={item?.book?.sample}
                                                                    target='_blank'
                                                                    className="border hover:text-white hover:bg-customFC6200 rounded-md py-[6px] px-3"
                                                                >
                                                                    View Sample
                                                                </Link>
                                                            ) : (
                    ...
                    <div className='relative mb-4 lg:mt-[-312px] h-max pb-[50px] bg-[#f8f9f8] rounded-md pt-[12px] px-[12px]'>
                        ...
                        <div className='px-[25px] pt-[35px] sm:text-[17px] text-sm'>
                            <div className='pb-[10px] sticky top-0 border-b'>
                                ...
                                {apiData?.customParam?.offerEndDate && (
                            ...
                            {apiData?.customParam?.books && (
                                <div className='py-[10px] border-b sm:text-[17px] text-sm'>
                                    <span className='flex items-center justify-between'>
                                        <span className='flex items-center gap-1'>
                                            <CgWebsite className='text-customFC6200' />
                                            <span className='text-[#9b9b9b]'><strong>Books</strong></span></span>
                                        <span className='text-[#606060]'>{apiData?.customParam?.books} Books</span>
                                    </span>
                                </div>
                            )}
                        </div>
...
export default PostalStudyDetails;