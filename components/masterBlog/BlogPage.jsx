        ...
        <div className='xl:grid xl:grid-cols-12'>
            ...
            {pageData?.advanceData?.data?.map((item, index) => (
              <div key={index} className='rounded-t mb-5 shadow-xl hover:shadow-2xl'>
                ...
                <div className='p-[25px]'>
                  ...
                  <Link 
                    href={`/blog/${item?.uri}`} 
                    className='mb-3 mt-3 text-[#221638] text-2xl hover:text-customFC6200 block font-semibold'
                    aria-label={`Read more about ${item?.title}`}
                  >
                    {item?.title}
                  </Link>
                  <div className='desci text-ellipsis line-clamp-5 text-[16px] text-[#212529]'>
                  ...
                  <div className='mt-[18px] flex items-center sm:gap-4 gap-2'>
                    ...
                    <div className='flex items-center'>
                      ...
                      <Link href={`/blog/${item?.uri}`} className='text-[#221638] hover:text-customFC6200 text-[15px]'>
          ...
          <div className='xl:col-span-4 col-span-12 xl:px-[15px]'>
            <div className='xl:pl-[15px] mb-10'>
              <div className='px-[25px] py-[30px] shadow-xl'>
                ...
                <div>
                  {pageData?.advanceData?.data?.slice(0, 4).map((item, index) => (
                    <div key={index} className='mb-[15px] pb-[15px] border-b'>
                      <div>
                        <div className='flex gap-5'>
                          <div>
                            <Link href={`/blog/${item?.uri}`}>
                              <span className='w-[80px] h-[80px] bg-center bg-cover block' style={{ backgroundImage: `url(${item?.featureImg})` }}></span>
                            </Link>
                          </div>
                          <div className='overflow-hidden'>
                            <span className='mt-[5px] mb-[3px] text-[#747474] block'>{formatDate(item?.publishedAt)}</span>
