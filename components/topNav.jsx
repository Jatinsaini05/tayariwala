              ...
              <Marquee>
                ...
                >
                  {topApi?.map((item, index) => (
                    <span className="flex items-center text-center gap-2 py-2" key={index}>
                      <FaRegHandPointRight />
                      {/* Foundation Course For UPSC CSE 2025/26/27 Admissions Open */}
                      {item.title}
                    </span>
                  ))}
                </Link>
                ...
                {topNavData?.menus?.TOP_MENU?.items.length
                  ? topNavData.menus.TOP_MENU.items.map((items, ind) => (
                      <Link href={items.url} className="" key={ind}>
                        <span className="btn1 bg-orange-600 hover:bg-[#071e63] transition ease-in-out  font-semi-medium md:font-medium pt-0.5 pb-2 px-3 text-sm md:text-lg rounded text-white">
                          {items.title}
                        </span>
                      </Link>
                    ))
              ...
              {topNavData?.menus?.PRIMARY_MENU?.items?.length
                ? topNavData.menus.PRIMARY_MENU.items.map((navList, ind) => (
                    ...
                    >
                      ...
                      {/* Subitems dropdown */}
                      {navList.subItems?.length ? (
                        <ul className="bg-[white] text-[black] w-48 absolute top-full left-0 z-10 py-3 px-1 text-sm hidden group-hover:block">
                  ...
                  ? topNavData.menus?.PRIMARY_MENU.items.map(
                      (mobNavItem, mobInd) => (
                        <li className="py-2" key={mobInd}>
                          <Link href={mobNavItem.url} onClick={()=>setNav(false)}>{mobNavItem.title}</Link>
