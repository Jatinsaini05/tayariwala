...
export const getStaticPaths = async () => {
  try {
    ...
    const coursePath = allCourseProgram.map((path, index) => ({
      params: {
        courseUri: path?.uri,
      },
    }));
    let paths = [...coursePath];
    // Return all paths
    return {
      paths,
    ...
    console.log("Failed to Load Course Program Path:", err);
    return {
      allCourseProgram: [],
    };
  }
...
export const getStaticProps = async (context) => {
  const {  courseUri } = context.params;
  try {
    let header = {
      apiHost: "https://vijethaiasacademyvja.com",
    };
    let response = await fetch(
      `https://v3.edkt.net/api/s/courseprogram-by-uri/${courseUri}`,
      {
        headers: header,
      }
    );
